...
class Solution {
    public int[] sortByBits(int[] arr) {
        return Arrays.stream(arr).boxed().sorted(Comparator.comparing(this::numOfOneBit).thenComparing(Integer::intValue)).mapToInt(Integer::intValue).toArray();
    }

    private int numOfOneBit(int num) {
        int res = 0;
        while (num > 0) {
            if ((num & 1) != 0) {
                res++;
