...
class Solution {
    public int getImportance(List<Employee> employees, int id) {
        Map<Integer, Employee> map = employees.parallelStream().collect(Collectors.toMap(e -> e.id, Function.identity(), (o, n) -> n));
    ...
    private int getImportanceHelper(Map<Integer, Employee> map, int id) {
        ...
        for (int subId : employee.subordinates) {
            res += getImportanceHelper(map, subId);
        }
        return res;
    }
}
